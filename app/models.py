...
class User(UserMixin, db.Model):
    ...
    )  # Filename of uploaded avatar image
    ...
    def to_dict(self):
        ...
        return {
            ...
            "avatar_url": f"/api/user/avatar/{self.avatar}" if self.avatar else None,
        }
...
class MagicLinkToken(db.Model):
    ...
    user = db.relationship(
        "User",
        backref=db.backref("magic_tokens", lazy=True, cascade="all, delete-orphan"),
    )
...
class UserContainer(db.Model):
    ...
    blocker = db.relationship("User", primaryjoin="UserContainer.blocked_by==User.id")
...
class AdminTakeoverSession(db.Model):
    ...
    target_user = db.relationship(
        ...
        backref=db.backref(
            "takeover_sessions_as_target", lazy=True, cascade="all, delete-orphan"
        ),
    )
